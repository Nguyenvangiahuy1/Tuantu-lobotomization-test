...
function NexoBFCast(skill, targets, myModel, myHRP)
    ...
    -- BF3 is intentionally allowed through even when CombatConfig does not
    -- contain SkillData.BlackFlashSkill3. The actual remote decides whether
    -- the skill is accepted by the current game build.
    if skill ~= "BlackFlashSkill3" and not NexoBFExists(skill) then return end
    local first = targets[1]:FindFirstChild("HumanoidRootPart")
    local dir = (first and (first.Position - myHRP.Position).Unit) or myHRP.CFrame.LookVector
...
local function NexoSelectedBlackFlashSkill()
    -- Kill Aura is explicitly mapped to BF3 first. Do not gate BF3 behind
    -- CombatConfig.SkillData existence: some game builds expose the remote
    -- skill without a matching client config entry.
    if NEXO_LV.KillOn then
        return "BlackFlashSkill3"
    end
    if NEXO_LV.BlackFlash3On then
        return "BlackFlashSkill3"
    end
